...
public partial class swap : System.Web.UI.Page
{
    ...
    private void GetSelectedRecord()
    {
        ...
        {
            RadioButton rb = (RadioButton)gvRoster.Rows[i].Cells[0].FindControl("rdoEmp2Swap");
            if (rb != null)
            {
                if (rb.Checked)
                {
                    HiddenField hf = (HiddenField)gvRoster.Rows[i].Cells[0].FindControl("hfEmp2Swap");
                    if (hf != null)
                    {
                        ViewState["Emp2Swap"] = hf.Value;
                    }
                    break;
                }
            }
    ...
    private void SetSelectedRecord()
    {
        for (int i = 0; i < gvRoster.Rows.Count; i++)
        {
            ...
            {
                HiddenField hf = (HiddenField)gvRoster.Rows[i].Cells[0].FindControl("hfEmp2Swap");
                if (hf != null && ViewState["Emp2Swap"] != null)
                {
                    if (hf.Value.Equals(ViewState["Emp2Swap"].ToString()))
                    {
                        rb.Checked = true;
                        break;
                    }
